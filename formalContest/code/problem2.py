...
def simulate_random_match(num_points, serve_win_prob=0.65):
    # 随机生成每个得分点的获胜方,发球方胜率较高
    wins = np.random.rand(num_points) < serve_win_prob
    momentum_scores = np.cumsum(wins)  # 累计胜点作为简化的势头得分
    return momentum_scores


actual_momentum_scores = data['momentum_score']

# 随机模拟N次比赛
N = 7000
simulate_scores = [simulate_random_match(len(actual_momentum_scores)) for _ in range(N)]

ks_state, p_value = stats.ks_2samp(actual_momentum_scores, np.concatenate(simulate_scores))
print(f"KS statistic: {ks_state}, P-value:{p_value}")

plt.hist(actual_momentum_scores, bins=30, alpha=0.5, label='Actual')
plt.hist(np.concatenate(simulate_scores), bins=30, alpha=0.5, label='Simulated', color='red')
